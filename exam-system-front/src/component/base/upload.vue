  ...
  methods: {
    ...
    submit() {
      this.detailData.forEach(item => {
        this.listFinal.push({
          teachername: item["老师名字"] ? item["老师名字"] : "",
          card: item["工号"] ? item["工号"] : "",
          password: '000000',
          college: item["学校"] ? item["学校"] : "",
          sex: item["性别"] ? item["性别"] : "",
      ...
      this.listFinal.forEach(item => {
        axios.get(this.url + "/teacher/addTeacher", {params:item}).then(res => {
          console.log(res);
        }).catch(err => {
            bool=false;
            throw err;
