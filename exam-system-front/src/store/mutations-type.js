// 试卷对象
// step1
export const SET_EXAMNAME = "SET_EXAMNAME";// 设置试卷名称
export const SET_STARTTIME = "SET_STARTTIME";// 设置发布考试时间
export const SET_CLASS = "SET_CLASS";//发布试卷的班级
export const SET_SHARE = "SET_SHARE";// 是否共享
// step2
export const SET_DESCRIPTION = "SET_DESCRIPTION";//试卷描述
export const SET_ENDTIME = "SET_ENDTIME";//截至日期时间
export const SET_CHECK = "SET_CHECK";//试卷是否限制时间
export const SET_TESTDURINGTIME = "SET_TESTDURINGTIME"//考试时间/分钟
export const SET_CHOICEONE = "SET_CHOICEONE"// 单选题
export const SET_CHOICEMANY = "SET_CHOICEMANY"// 多选题
export const SET_JUDGETEST = "SET_JUDGETEST"// 判断题
export const SET_FEEDFULL = "SET_FEEDFULL"// 填空题
export const SET_SHORTANSWER = "SET_SHORTANSWER"// 简答题
export const SET_TRYTIMES = "SET_TRYTIMES"//尝试次数
// step3
export const SET_QUESTIONLIST = "SET_QUESTIONLIST";// 题目列表


// 设置发布试卷进行到的阶段
export const SET_STEP = "SET_STEP";// 设置试卷名称

// 班级对象
export const SET_CLASSINFO = "SET_CLASSINFO";// 设置班级信息

// 用户对象
export const SET_USERINFO = "SET_USERINFO"// 设置用户信息